...
function getSessionUser(store, token) {
  ...
  const sessionMs = SESSION_DAYS * 24 * 60 * 60 * 1000;
  if (new Date(session.expiresAt).getTime() - current < sessionMs / 2) {
    session.expiresAt = new Date(current + sessionMs).toISOString();
    writeStore(store);
  }
  return user;
}
