...
- 高速捡漏：持续检查空位，发现候选后立即尝试预约。
- 候选抢座：每轮只尝试最有希望的一小批座位，避免遍历所有空位拖慢速度。
- 小并发尝试：每批可同时尝试 1-4 个座位，默认 2。
- 失败冷却：失败座位短暂跳过，避免死磕同一个假空位。
...
- 默认捡漏间隔是 0.5 秒，并发数是 2，每轮候选是 6。继续调高可能更快，但也更容易触发限流或账号风控。
- 如果日志里经常出现“座位没人但失败”，通常是座位状态延迟、他人同时抢占、接口判定为假空位或账号限制；可把失败冷却调高到 1.5-2 秒，减少反复撞同一座位。
